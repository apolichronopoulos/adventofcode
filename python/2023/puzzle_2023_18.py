...
def solve(part=1, case=1, start=None):
    ...
    if part == 1:
            ...
            for j in range(len(tiles[i])):
                ...
                if c == "#" and last_c_row != "#":  # open
                    tU = (
                        path_current != -1
                        and path_U != -1
                        and abs(path_current - path_U) == 1
                    )
                    tD = (
                        path_current != -1
                        and path_D != -1
                        and abs(path_current - path_D) == 1
                    )
        ...
        for i, j in path:
            ...
            max_j_per_i[i] = max(
                max_j_per_i[i] if i in max_j_per_i else -sys.maxsize, j
            )
            min_i_per_j[j] = min(min_i_per_j[j] if j in min_i_per_j else sys.maxsize, i)
            max_i_per_j[j] = max(
                max_i_per_j[j] if j in max_i_per_j else -sys.maxsize, i
            )
            min_i = min(i, min_i)
            min_j = min(j, min_j)
            max_i = max(i, max_i)
        ...
        print(
            f"vertical: {vertical}, horizontal: {horizontal}, top size: {vertical * horizontal}"
        )
